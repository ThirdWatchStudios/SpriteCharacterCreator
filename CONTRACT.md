...
- `wings[]`: `{ id, departmentId, label, roomIds[], bounds:{x,y,cols,rows} }` — rooms grouped into department wings (the physical projection of the org chart; sim reveals wing-by-wing + measures wing distance). Each `rooms[]` entry may carry an optional `departmentId` (the F2.1 catalog id) that `computeWings` groups by. **Backward-compatible:** a single-office template with no department-tagged rooms exports exactly one implicit wing (`wing-main`, `departmentId: null`) covering every room; department wings appear once the cascade/multi-wing templates tag rooms, alongside a `wing-common` catch-all for shared rooms.
- `connectivity[]`: `{ wings: [wingIdA, wingIdB], doorways }` — the wing-adjacency graph derived from the door topology (F1.3). Each edge is an undirected wing-id pair (sorted) that a doorway physically joins, with `doorways` = how many doorways connect them (a connection-strength/weight hint). The sim reads the edge list as a graph: BFS from the entry wing yields both **fog-of-war reveal order** and **wing-to-wing (hop) distance**. A single-office layout has one wing and an empty `connectivity[]`; a composed multi-wing office connects every department wing to the common (hallway) wing, so the graph is connected.
- **Department-tagged spawn (F3.4):** when the project carries a generated population (promoted cast with department profiles), the office auto-derives a wing per department present in the cast and seats each generated agent at a chair **in its department's wing** (`characterSpawns[]` land inside the matching wing room), leaving per-wing transfer headroom; over-capacity is reported, not silently overflowed. The scripted base cast keep their authored spawns. No schema change — only spawn *placement* changes.
- **Footprint scaling (F1.4):** `cols`/`rows` are not fixed. The single-office templates stay 22×14, but when the generator is asked for department wings (`generateOfficeLayout(..., { wingDepartmentIds })`) it composes a wider footprint — a full-width hallway band + reception core + one cubicle-farm wing per department — so `cols` grows with department count (`8 + N*8`) while `rows` stays 14. Wings share single walls (inclusive overlap-by-1, no double walls/gaps) and each connects to the corridor by a single-tile doorway, so the shared-edge/single-doorway/badge-pairing invariants hold across wings exactly as within a single office. No schema bump — the v2 payload already carries `cols`/`rows` + `rooms[].departmentId`.
